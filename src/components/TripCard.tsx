    ...
    >
      ...
      <div className="flex flex-col gap-3 p-5">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>{trip.distance.toLocaleString()} km · {trip.steps.length} stops</span>
        </div>

        <div className="mt-1 flex flex-wrap gap-1.5">
          {stepLocations.slice(0, 4).map((loc) => (
            <span key={loc} className="rounded-full bg-secondary px-2.5 py-0.5 text-xs font-medium text-secondary-foreground">
              {loc}
            </span>
          ))}
